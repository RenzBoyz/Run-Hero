...
module.exports = welcome = async (dha, anu) => {
		...
		try {
            ...
            dha.sendMessage(anu.jid, 'Assalamualaikum', 'conversation')
            }
             if (anu.action == 'add' && !mem.includes(dha.user.jid)) {
             if (!welkom.includes(anu.jid)) return
	            ...
	            buff = fs.readFileSync(`media/welcome.jpg`) 
                buttons = [{buttonId: `#y`,buttonText:{displayText: 'Intro Dulu🛐'},type:1}, {buttonId: `#poasp`,buttonText:{displayText: 'Salam Kenal🤝'},type:1}, {buttonId: `#pasp`,buttonText:{displayText: 'Semoga Betah🙏'},type:1}]
                imageMsg = (await dha.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${teks}`, footerText: '*Join Link Di Bawah Ini*\nhttps://chat.whatsapp.com/BDp31ywNvXuA5LMoEaKwyD', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
            ...
            if (!welkom.includes(anu.jid)) return
                ...
                memeg = mdata.participants.length
                out = `Byee ${anu_user}`
                buff = fs.readFileSync(`media/out.jpg`) 
                buttons = [{buttonId: `#y`,buttonText:{displayText: 'Sayonara👋'},type:1}]
                imageMsg = (await dha.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${out}`, footerText: 'Join Link Di Bawah Ini\nhttps://chat.whatsapp.com/BDp31ywNvXuA5LMoEaKwyD', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
                prep = await dha.prepareMessageFromContent(mdata.id,{buttonsMessage},{})
                dha.relayWAMessage(prep)
            }
		} catch (e) {
